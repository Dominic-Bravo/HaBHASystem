...
namespace HaBHAServer.NewControllers
{
    ...
    public class AppImageController : ControllerBase
    {
        ...
        [HttpPost("JsonAddImages")]
        public async Task<IActionResult> AddJsonAppImage([FromForm] string request, [FromForm] IFormFile image)
        {
            try
            {
                var appImage = JsonConvert.DeserializeObject<AppImage>(request);

                if (appImage == null || string.IsNullOrWhiteSpace(appImage.Description))
                {
                ...
                var imageId = _appImageRepository.AddAppImage(
                    appImage.BoardinghouseId,
                    appImage.QRCodeImageId,
                    appImage.UserId,
                    imageData,
                ...
                return imageId > 0
                    ? Ok(new { Message = "Image added successfully", ImageId = imageId })
                    : BadRequest(new { Message = "Failed to add image." });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { Message = "Internal server error", Details = ex.Message });
